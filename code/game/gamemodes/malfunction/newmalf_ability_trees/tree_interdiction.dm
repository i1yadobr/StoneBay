	...
	if(target)
		...
		if(target && target.lockcharge)
			...
			if(target.lockcharge)
				to_chat(user, SPAN("notice", "Unlock Failed, lockdown wire cut."))
				to_chat(target, SPAN("notice", "Unlock Failed, lockdown wire cut."))
			else
...
/datum/game_mode/malfunction/verb/hack_cyborg(mob/living/silicon/robot/target as mob in get_unlinked_cyborgs(usr))
	...
	if(!L.len)
		to_chat(user, SPAN("notice", "ERROR: No unlinked cyborgs detected!"))
...
/datum/game_mode/malfunction/verb/hack_ai(mob/living/silicon/ai/target as mob in get_other_ais(usr))
	...
	if(!L.len)
		to_chat(user, SPAN("notice", "ERROR: No other AIs detected!"))
	if(target && !istype(target))
