...
/obj/structure/deity/blood_forge/proc/craft_item(path, blood_cost, mob/user)
	if(busy)
		to_chat(user, SPAN("warning", "Someone is already using \the [src]!"))
		return

	busy = 1
	to_chat(user, SPAN("notice", "You dip your hands into \the [src]'s [text_modifications["Dip"]]"))
...
/obj/structure/deity/blood_stone/attack_hand(mob/user)
	...
	while(do_after(H, 50, src))
		user.audible_message("\The [user] utters something under their breath.", SPAN("cult", "You mutter a dark prayer to your master as you feel the stone eat away at your lifeforce."))
