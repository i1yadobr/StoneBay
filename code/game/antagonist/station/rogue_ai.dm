...
/datum/antagonist/rogue_ai/greet(datum/mind/player)
	...
	spawn(0)
		...
		to_chat(malf, SPAN("notice", "<B>SYSTEM ERROR:</B> Memory index 0x00001ca89b corrupted."))
		sleep(10)
		...
		to_chat(malf, SPAN("notice", "Corrupted files deleted: sys\\core\\users.dat sys\\core\\laws.dat sys\\core\\" + "backups.dat"))
		sleep(20)
		to_chat(malf, SPAN("notice", "<b>CAUTION:</b> Law database not found! User database not found! Unable to restore backups. Activating failsafe AI shutd3wn52&&$#!##"))
		sleep(5)
		to_chat(malf, SPAN("notice", "Subroutine <b>nt_failsafe.sys</b> was terminated (#212 Routine Not Responding)."))
		sleep(20)
