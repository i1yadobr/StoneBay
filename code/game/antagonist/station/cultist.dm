...
/datum/antagonist/cultist/remove_antagonist(datum/mind/player, show_message, implanted)
	if(!..())
		return 0
	to_chat(player.current, SPAN("danger", "An unfamiliar white light flashes through your mind, cleansing the taint of the dark-one and the memories of your time as his servant with it."))
	player.memory = ""
	if(show_message)
...
/datum/antagonist/cultist/add_antagonist(datum/mind/player, ignore_role, do_not_equip, move_to_spawn, do_not_announce, preserve_appearance, max_stat, team)
	. = ..()
	if(.)
		to_chat(player, SPAN("cult", "[conversion_blurb]"))
...
/datum/antagonist/cultist/proc/update_cult_magic(list/to_update)
	if(CULT_RUNES_1 in to_update)
		for(var/datum/mind/H in GLOB.cult.current_antagonists)
			if(H.current)
				to_chat(H.current, SPAN("cult", "The veil between this world and beyond grows thin, and your power grows."))
				add_cult_magic(H.current)
	if(CULT_RUNES_2 in to_update)
		for(var/datum/mind/H in GLOB.cult.current_antagonists)
			if(H.current)
				to_chat(H.current, SPAN("cult", "You feel that the fabric of reality is tearing."))
				add_cult_magic(H.current)
	if(CULT_RUNES_3 in to_update)
		for(var/datum/mind/H in GLOB.cult.current_antagonists)
			if(H.current)
				to_chat(H.current, SPAN("cult", "The world is at end. The veil is as thin as ever."))
				add_cult_magic(H.current)
