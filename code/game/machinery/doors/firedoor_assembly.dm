...
/obj/structure/firedoor_assembly/attackby(obj/item/C, mob/user)
	if(isCoil(C) && !wired && anchored)
		...
		if (cable.get_amount() < 1)
			to_chat(user, SPAN("warning", "You need one length of coil to wire \the [src]."))
			return
		...
		if(do_after(user, 40, src))
			if(!src) return
			to_chat(user, SPAN("notice", "You cut the wires!"))
		...
		if(anchored)
			...
			to_chat(user, SPAN("warning", "You must secure \the [src] first!"))
	else if(isWrench(C))
		anchored = !anchored
