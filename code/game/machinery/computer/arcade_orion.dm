	...
	switch(event)
		if(ORION_TRAIL_RAIDERS)
			if(istype(usr,/mob/living/carbon))
				...
				to_chat(usr, SPAN("warning", "The sounds of battle fill your ears..."))
		if(ORION_TRAIL_ILLNESS)
			if(istype(usr,/mob/living/carbon/human))
				...
				to_chat(usr, SPAN("warning", "You feel ill."))
		if(ORION_TRAIL_CARP)
			to_chat(usr, SPAN("danger", " Something bit you!"))
		...
		if(ORION_TRAIL_FLUX)
			if(istype(usr,/mob/living/carbon) && prob(75))
				...
				to_chat(usr, SPAN("warning", "A violent gale blows past you, and you barely manage to stay standing!"))
		if(ORION_TRAIL_MALFUNCTION)
			if(supplies["3"])
				return
		...
		if(ORION_TRAIL_GAMEOVER)
			to_chat(usr, SPAN("danger", "<font size=3>You're never going to make it to Orion...</font>"))
			var/mob/living/M = usr
...
/obj/item/orion_ship/attack_self(mob/user)
	...
	src.audible_message("<b>\The [src]</b> says, 'This is ship ID #[rand_ship_id] to Orion Port Authority. We're coming in for landing, over.'", runechat_message = "This is ship ID #[rand_ship_id] to Orion Port Authority. We're coming in for landing, over.")
	sleep(20)
	src.visible_message(SPAN("warning", "[src] begins to vibrate..."))
	src.audible_message("<b>\The [src]</b> says, 'Uh, Port? Having some issues with our reactor, could you check it out? Over.'", runechat_message = "Uh, Port? Having some issues with our reactor, could you check it out? Over.")
	sleep(30)
	src.audible_message("<b>\The [src]</b> says, 'Oh, God! Code Eight! CODE EIGHT! IT'S GONNA BL-'", runechat_message = "Oh, God! Code Eight! CODE EIGHT! IT'S GONNA BL-")
	sleep(3.6)
