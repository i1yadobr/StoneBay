...
/obj/machinery/portable_atmospherics/attackby(obj/item/W as obj, mob/user as mob)
	if ((istype(W, /obj/item/tank) && !( src.destroyed )))
		...
		if(connected_port)
			disconnect()
			to_chat(user, SPAN("notice", "You disconnect \the [src] from the port."))
			update_icon()
			return
		else
			...
			if(possible_port)
				if(connect(possible_port))
					to_chat(user, SPAN("notice", "You connect \the [src] to the port."))
					update_icon()
					return
				else
					to_chat(user, SPAN("notice", "\The [src] failed to connect to the port."))
					return
			else
				to_chat(user, SPAN("notice", "Nothing happens."))
				return
...
/obj/machinery/portable_atmospherics/powered/attackby(obj/item/I, mob/user)
	...
	if(isScrewdriver(I))
		if(!cell)
			to_chat(user, SPAN("warning", "There is no power cell installed."))
			return
