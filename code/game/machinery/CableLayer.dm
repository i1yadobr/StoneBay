...
/obj/machinery/cablelayer/attack_hand(mob/user)
	if(!cable&&!on)
		to_chat(user, SPAN("warning", "\The [src] doesn't have any cable loaded."))
		return
	on=!on
...
/obj/machinery/cablelayer/attackby(obj/item/O, mob/user)
	if(isCoil(O))
		...
		if(!result)
			to_chat(user, SPAN("warning", "\The [src]'s cable reel is full."))
		else
	...
	if(isWirecutter(O))
		if(cable && cable.amount)
			...
			to_chat(usr, SPAN("warning", "There's no more cable on the reel."))
