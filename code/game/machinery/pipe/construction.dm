...
/obj/item/pipe/attackby(obj/item/W as obj, mob/user as mob)
	...
	for(var/obj/machinery/atmospherics/M in src.loc)
		if((M.initialize_directions & pipe_dir) && M.check_connect_types_construction(M,src))	// matches at least one direction on either type of pipe & same connection type
			to_chat(user, SPAN("warning", "There is already a pipe of the same type at this location."))
			return 1
	// no conflicts found

	var/pipefailtext = SPAN("warning", "There's nothing to connect this pipe section to!") //(with how the pipe code works, at least one end needs to be connected to something, otherwise the game deletes the segment)"
...
///// Z-Level stuff
	...
	user.visible_message( \
		...
		SPAN("notice", "You have fastened the [src]."), \
		"You hear ratchet.")
...
/obj/item/pipe_meter/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(!locate(/obj/machinery/atmospherics/pipe, src.loc))
		to_chat(user, SPAN("warning", "You need to fasten it to a pipe"))
		return 1
	new /obj/machinery/meter( src.loc )
	playsound(src.loc, 'sound/items/Ratchet.ogg', 50, 1)
	to_chat(user, SPAN("notice", "You have fastened the meter to the pipe"))
	qdel(src)
