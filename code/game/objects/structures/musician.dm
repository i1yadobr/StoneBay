...
/obj/structure/device/piano/attackby(obj/item/O as obj, mob/user as mob)
	if(isWrench(O))
		if (anchored)
			...
			to_chat(user, SPAN("notice", "You begin to loosen \the [src]'s casters..."))
			if (do_after(user, 40, src))
				user.visible_message(
					"[user] loosens \the [src]'s casters.",
					SPAN("notice", "You have loosened \the [src]. Now it can be pulled somewhere else."),
					"You hear ratchet.")
				src.anchored = 0
		else
			playsound(src.loc, 'sound/items/Ratchet.ogg', 50, 1)
			to_chat(user, SPAN("notice", "You begin to tighten \the [src] to the floor..."))
			if (do_after(user, 20, src))
				user.visible_message(
					"[user] tightens \the [src]'s casters.",
					SPAN("notice", "You have tightened \the [src]'s casters. Now it can be played again."),
					"You hear ratchet.")
				src.anchored = 1
	else
		..()
