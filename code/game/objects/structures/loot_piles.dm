	...
	if(isliving(user))
		...
		if(do_after(user, rand(4 SECONDS,6 SECONDS), src))
			...
			if(loot_depletion && loot_left <= 0)
				to_chat(L, SPAN("warning", "\The [src] has been picked clean."))
				busy = FALSE
				return

			//You already searched this one
			if( (user.ckey in searched_by) && !allow_multiple_looting)
				to_chat(L, SPAN("warning", "You can't find anything else vaguely useful in \the [src].  Another set of eyes might, however."))
				busy = FALSE
				return

			// You got unlucky.
			if(chance_nothing && prob(chance_nothing))
				to_chat(L, SPAN("warning", "Nothing in this pile really catches your eye..."))
			...
			if(loot)
				...
				to_chat(L, SPAN("[span]", "You found \a [loot]!"))
				if(loot_depletion)
					loot_left--
					if(loot_left <= 0)
						to_chat(L, SPAN("warning", "You seem to have gotten the last of the spoils inside \the [src]."))
