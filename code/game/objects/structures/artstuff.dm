...
/obj/item/canvas/proc/finalize(mob/user, is_copy = FALSE)
	...
	if(!epicenter && !author_ckey)
		return
	message_admins("The [is_copy ? "copy of" : "new"] art has been created by [author_ckey] in <A HREF='byond://?_src_=holder;adminplayerobservecoodjump=1;X=[epicenter.x];Y=[epicenter.y];Z=[epicenter.z]'>(x:[epicenter.x], y:[epicenter.y], z:[epicenter.z])</a>")
