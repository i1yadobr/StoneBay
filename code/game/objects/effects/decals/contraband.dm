...
/obj/item/contraband/poster/afterattack(atom/A, mob/user, adjacent, clickparams)
	...
	if (!(placement_dir in GLOB.cardinal))
		to_chat(user, SPAN("warning", "You must stand directly in front of the wall you wish to place that on."))
		return
	...
	if (stuff_on_wall)
		to_chat(user, SPAN("notice", "There is already a poster there!"))
		return

	to_chat(user, SPAN("notice", "You start placing the poster on the wall...")) //Looks like it's uncluttered enough. Place the poster.
	...
	spawn(17)
		...
		if(iswall(W) && user && P.loc == user.loc) //Let's check if everything is still there
			to_chat(user, SPAN("notice", "You place the poster!"))
		else
...
/obj/structure/sign/poster/attackby(obj/item/W as obj, mob/user as mob)
	if(isWirecutter(W))
		...
		if(ruined)
			to_chat(user, SPAN("notice", "You remove the remnants of the poster."))
			qdel(src)
		else
			to_chat(user, SPAN("notice", "You carefully remove the poster from the wall."))
