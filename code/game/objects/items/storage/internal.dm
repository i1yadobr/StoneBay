...
/obj/item/storage/internal/proc/handle_attack_hand(mob/user as mob)
	...
	if(ishuman(user))
		...
		if(H.l_store == master_item && (!H.get_active_hand() || !H.get_passive_hand()))	//Prevents opening if it's in a pocket.
			if(H.put_in_hands(master_item))
				H.l_store = null
			return 0
		if(H.r_store == master_item && (!H.get_active_hand() || !H.get_passive_hand()))
			if(H.put_in_hands(master_item))
				H.r_store = null
			return 0
