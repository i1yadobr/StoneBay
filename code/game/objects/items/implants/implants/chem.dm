...
/obj/item/implant/chem/activate(amount)
	...
	to_chat(R, SPAN("notice", "You hear a faint *beep*."))
	...
	if(istype(I, /obj/item/reagent_containers/syringe))
		if(reagents.total_volume >= reagents.maximum_volume)
			...
			if(do_after(user, 5, src))
				...
				to_chat(user, SPAN("notice", "You inject 5 ml of the solution. The syringe now contains [I.reagents.total_volume] ml."))
	else
		..()
