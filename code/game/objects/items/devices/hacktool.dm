...
/obj/item/device/multitool/hacktool/proc/attempt_hack(mob/user, atom/target)
	if(is_hacking)
		to_chat(user, SPAN("warning", "You are already hacking!"))
		return 1
	if(!is_type_in_list(target, supported_types))
		to_chat(user, "\icon[src] [SPAN("warning", "Unable to hack this target.")]")
		return 0
	...
	to_chat(user, SPAN("notice", "You begin hacking \the [target]..."))
	is_hacking = 1
	...
	if(hack_result && in_hack_mode)
		to_chat(user, SPAN("notice", "Your hacking attempt was succesful!"))
		...
		to_chat(user, SPAN("warning", "Your hacking attempt failed!"))
	return 1
