...
/obj/item/device/suit_sensor_jammer/attackby(obj/item/I as obj, mob/user as mob)
	if(isCrowbar(I))
		if(bcell)
			to_chat(user, SPAN("notice", "You remove \the [bcell]."))
			disable()
			bcell.dropInto(loc)
			bcell = null
		else
			to_chat(user, SPAN("warning", "There is no cell to remove."))
	else if(istype(I, /obj/item/cell))
		if(bcell)
			to_chat(user, SPAN("warning", "There's already a cell in \the [src]."))
		else if(user.drop(I, src))
			bcell = I
			to_chat(user, SPAN("notice", "You insert \the [bcell] into \the [src].."))
		else
			to_chat(user, SPAN("warning", "You're unable to insert the battery."))
