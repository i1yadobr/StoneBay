...
/obj/item/aicard/Topic(href, href_list, state)
	...
	if (href_list["radio"])
		...
		to_chat(carded_ai, SPAN("warning", "Your Subspace Transceiver has been [carded_ai.ai_radio.disabledAi ? "disabled" : "enabled"]!"))
		to_chat(user, SPAN("notice", "You [carded_ai.ai_radio.disabledAi ? "disable" : "enable"] the AI's Subspace Transceiver."))
	if (href_list["wireless"])
		carded_ai.control_disabled = text2num(href_list["wireless"])
		to_chat(carded_ai, SPAN("warning", "Your wireless interface has been [carded_ai.control_disabled ? "disabled" : "enabled"]!"))
		to_chat(user, SPAN("notice", "You [carded_ai.control_disabled ? "disable" : "enable"] the AI's wireless interface."))
		update_icon()
	return 1
