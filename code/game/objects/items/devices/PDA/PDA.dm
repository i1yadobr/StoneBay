...
/obj/item/device/pda/ai/verb/cmd_toggle_pda_receiver()
	...
	if(usr.stat == 2)
		...
		return
	toff = !toff
	to_chat(usr, SPAN("notice", "PDA sender/receiver toggled [(toff ? "Off" : "On")]!"))
...
/obj/item/device/pda/ai/verb/cmd_toggle_pda_silent()
	...
	to_chat(usr, SPAN("notice", "PDA ringer toggled [(message_silent ? "Off" : "On")]!"))
...
/obj/item/device/pda/proc/create_message(mob/living/U = usr, obj/item/device/pda/P, tap = 1)
	if(!istype(P))
		to_chat(U, SPAN("notice", "ERROR: This user does not accept messages."))
		return
	if(tap)
	...
	if(!get_message_server(z))
		to_chat(U, SPAN("notice", "ERROR: Messaging server is not responding."))
		tempmessage[P] = message
		return
	if(!get_message_server(P.z))
		to_chat(U, SPAN("notice", "ERROR: Receiving messaging server is not responding."))
		tempmessage[P] = message
		return
		...
		for(var/mob/M in GLOB.player_list)
			if(M.is_ooc_dead() && M.get_preference_value(/datum/client_preference/ghost_radio) == GLOB.PREF_ALL_CHATTER) // src.client is so that ghosts don't have to listen to mice
				if(istype(M, /mob/new_player))
					continue
				M.show_message(SPAN("game say", "PDA Message - [SPAN("name", "[owner]")] -> [SPAN("name", "[P.owner]")]: [SPAN("message", "[message]")]"))
...
/obj/item/device/pda/ai/new_message(atom/movable/sending_unit, sender, sender_job, message)
	if(!istype(sending_unit))
		to_chat(usr, SPAN("bad", "This destination does not accept messages."))
		return
	var/track = ""
...
/obj/item/device/pda/verb/verb_remove_id()
	...
	if(id)
		...
		to_chat(usr, SPAN("notice", "\The [src] does not have an ID in it."))
...
/obj/item/device/pda/attackby(obj/item/C as obj, mob/user as mob)
	...
	if(istype(C, /obj/item/cartridge) && !cartridge)
		if(!user.drop(C, src))
			return
		cartridge = C
		to_chat(user, SPAN("notice", "You insert [cartridge] into [src]."))
		...
		if(!owner)
			...
			if(((src in user.contents) && (C in user.contents)) || (istype(loc, /turf) && in_range(src, user) && (C in user.contents)) )
				if(id_check(user, 2))
					to_chat(user, SPAN("notice", "You put the ID into \the [src]'s slot."))
		...
		if(!user.drop(C, src))
			return
		pai = C
		to_chat(user, SPAN("notice", "You slot \the [C] into [src]."))
		SSnano.update_uis(src) // update all UIs attached to src
		...
		if(O)
			...
			to_chat(user, SPAN("notice", "You slide \the [C] into \the [src]."))
	return
		...
		switch(scanmode)
			...
			if(2)
				if (!istype(C:dna, /datum/dna))
					to_chat(user, SPAN("notice", "No fingerprints found on [C]"))
				else
					to_chat(user, SPAN("notice", "\The [C]'s Fingerprints: [md5(C:dna.uni_identity)]"))
				if ( !(C:blood_DNA) )
					to_chat(user, SPAN("notice", "No blood found on [C]"))
					if(C:blood_DNA)
						qdel(C:blood_DNA)
				else
					to_chat(user, SPAN("notice", "Blood found on [C]. Analysing..."))
					spawn(15)
						for(var/blood in C:blood_DNA)
							to_chat(user, SPAN("notice", "Blood type: [C:blood_DNA[blood]]\nDNA: [blood]"))
			...
			if(4)
				...
				user.show_message(SPAN("notice", "Radiation dose: [fmt_siunit(C.radiation, "Sv", 3)]"))
...
/obj/item/device/pda/afterattack(atom/A as mob|obj|turf|area, mob/user as mob, proximity)
		...
		if(3)
			...
			if(!QDELETED(A.reagents))
				if(A.reagents.reagent_list.len > 0)
					...
					to_chat(user, SPAN("notice", "[reagents_length] chemical agent[reagents_length > 1 ? "s" : ""] found."))
					for (var/re in A.reagents.reagent_list)
						to_chat(user, SPAN("notice", "    [re]"))
				else
					to_chat(user, SPAN("notice", "No active chemical agents found in [A]."))
			else
				to_chat(user, SPAN("notice", "No significant chemical agents found in [A]."))
	...
	if (!scanmode && istype(A, /obj/item/paper) && owner)
		...
		// Inform the user
		to_chat(user, SPAN("notice", "Paper scanned and OCRed to notekeeper."))//concept of scanning paper copyright brainoblivion 2009
