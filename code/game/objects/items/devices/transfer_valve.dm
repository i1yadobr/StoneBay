/obj/item/device/transfer_valve
	...
	description_info = "This machine is used to merge the contents of two different gas tanks. Plug the tanks into the transfer, \
	then open the valve to mix them together. You can also attach various assembly devices to trigger this process."
	description_antag = "With a tank of hot plasma and cold oxygen, this benign little atmospheric device becomes an incredibly deadly bomb. \
...
/obj/item/device/transfer_valve/attackby(obj/item/item, mob/user)
	...
	if (!isturf(loc) && !ismob(loc))
		to_chat(user, SPAN("warning", "There is not enough space to do assembly."))
		return
	...
	if(istype(item, /obj/item/tank))
		...
		if(tank_one && tank_two)
			to_chat(user, SPAN("warning", "There are already two tanks attached, remove one first."))
			return
		...
		to_chat(user, SPAN("notice", "You attach the tank to the transfer valve."))
...
//TODO: Have this take an assemblyholder
		...
		if(A.secured)
			to_chat(user, SPAN("notice", "The device is secured."))
			return
		if(attached_device)
			to_chat(user, SPAN("warning", "There is already an device attached to the valve, remove it first."))
			return
		if(A.proximity_monitor)
			A.proximity_monitor.set_host(src, A)
		user.drop(item, src)
		attached_device = A
		to_chat(user, SPAN("notice", "You attach the [item] to the valve controls and secure it."))
		A.holder = src
