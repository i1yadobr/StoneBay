...
/obj/machinery/radiocarbon_spectrometer/attackby(obj/I as obj, mob/user as mob)
	if(scanning)
		to_chat(user, SPAN("warning", "You can't do that while [src] is scanning!"))
	else
		...
		if(istype(I, /obj/item/reagent_containers/vessel))
			...
			if(choice == "Add coolant")
				...
				to_chat(user, SPAN("info", "You empty [amount_transferred]u of coolant into [src]."))
				update_coolant()
				return
				...
				to_chat(user, SPAN("info", "You remove [amount_transferred]u of coolant from [src]."))
				update_coolant()
				return
		if(scanned_item)
			to_chat(user, SPAN("warning", "\The [src] already has \a [scanned_item] inside!"))
			return
		if(!user.drop(I, src))
			return
		scanned_item = I
		to_chat(user, SPAN("notice", "You put \the [I] into \the [src]."))
	...
	if(scanning)
		if(!scanned_item || scanned_item.loc != src)
			...
			if(scanner_seal_integrity <= 0 || (scanner_temperature >= 1273 && !rad_shield))
				stop_scanning()
				src.visible_message(SPAN("notice", "\icon[src] buzzes unhappily. It has failed mid-scan!"))

			if(prob(5))
				src.visible_message(SPAN("notice", "\icon[src] [pick("whirrs","chuffs","clicks")][pick(" excitedly"," energetically"," busily")]."))
	else
		//gradually cool down over time
		if(scanner_temperature > 0)
			scanner_temperature = max(scanner_temperature - 5 - 10 * rand(), 0)
		if(prob(0.75))
			src.visible_message(SPAN("notice", "\icon[src] [pick("plinks","hisses")][pick(" quietly"," softly"," sadly"," plaintively")]."))
		...
		if(scanning)
			...
			if(scanned_item)
				if(scanner_seal_integrity > 0)
					...
					to_chat(user, SPAN("notice", "Scan initiated."))
				else
					to_chat(user, SPAN("warning", "Could not initiate scan, seal requires replacing."))
			else
				to_chat(user, SPAN("warning", "Insert an item to scan."))
		. = TOPIC_REFRESH
