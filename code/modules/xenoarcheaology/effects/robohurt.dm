...
/datum/artifact_effect/robohurt/DoEffectTouch(mob/user)
	if(user)
		if (istype(user, /mob/living/silicon/robot))
			...
			to_chat(R, SPAN("danger", "Your systems report severe damage has been inflicted!"))
	...
	if(holder)
		...
		for (var/mob/living/silicon/robot/M in range(src.effectrange,T))
			if(world.time - last_message > 200)
				to_chat(M, SPAN("danger", "SYSTEM ALERT: Harmful energy field detected!"))
...
/datum/artifact_effect/robohurt/DoEffectPulse()
	if(holder)
		...
		for (var/mob/living/silicon/robot/M in range(src.effectrange,T))
			if(world.time - last_message > 200)
				to_chat(M, SPAN("danger", "SYSTEM ALERT: Structural damage inflicted by energy pulse!"))
				last_message = world.time
