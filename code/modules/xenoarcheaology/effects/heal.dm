...
/datum/artifact_effect/heal/DoEffectTouch(mob/toucher)
	...
	if(toucher && iscarbon(toucher))
		...
		if(prob(weakness * 100))
			...
			to_chat(C, SPAN("notice", "You feel a soothing energy invigorate you."))
...
/datum/artifact_effect/heal/DoEffectAura()
	...
	if(holder)
		...
		for (var/mob/living/carbon/C in range(src.effectrange,T))
			...
			if(prob(weakness * 100))
				if(prob(10))
					to_chat(C, SPAN("notice", "You feel a soothing energy radiating from something nearby."))
...
/datum/artifact_effect/heal/DoEffectPulse()
	...
	if(holder)
		...
		for (var/mob/living/carbon/C in range(src.effectrange,T))
			var/weakness = GetAnomalySusceptibility(C)
			if(prob(weakness * 100))
				to_chat(C, SPAN("notice", "A wave of energy invigorates you."))
