...
/obj/structure/table/attackby(obj/item/W, mob/user)
	...
	if(carpeted && isCrowbar(W))
		user.visible_message(SPAN("notice", "\The [user] removes the carpet from \the [src]."),
		                              SPAN("notice", "You remove the carpet from \the [src]."))
	...
	if(!carpeted && material && istype(W, /obj/item/stack/tile/carpet))
		...
		if(C.use(1))
			user.visible_message(SPAN("notice", "\The [user] adds \the [C] to \the [src]."),
			                              SPAN("notice", "You add \the [C] to \the [src]."))
			carpeted = 1
			update_icon()
			return 1
		else
			to_chat(user, SPAN("warning", "You don't have enough carpet!"))
...
/obj/structure/table/proc/common_material_add(obj/item/stack/material/S, mob/user, verb) // Verb is actually verb without 'e' or 'ing', which is added. Works for 'plate'/'plating' and 'reinforce'/'reinforcing'.
	...
	if(!istype(M))
		to_chat(user, SPAN("warning", "You cannot [verb]e \the [src] with \the [S]."))
		return null

	if(manipulating) return M
	manipulating = 1
	to_chat(user, SPAN("notice", "You begin [verb]ing \the [src] with [M.display_name]."))
...
/obj/structure/table/proc/common_material_remove(mob/user, material/M, delay, what, type_holding, sound)
	if(!M.stack_type)
		to_chat(user, SPAN("warning", "You are unable to remove the [what] from this table!"))
		return M

	if(manipulating) return M
	manipulating = 1
	user.visible_message(SPAN("notice", "\The [user] begins removing the [type_holding] holding \the [src]'s [M.display_name] [what] in place."),
	                              SPAN("notice", "You begin removing the [type_holding] holding \the [src]'s [M.display_name] [what] in place."))
	if(sound)
	...
	user.visible_message(SPAN("notice", "\The [user] removes the [M.display_name] [what] from \the [src]."),
	                              SPAN("notice", "You remove the [M.display_name] [what] from \the [src]."))
...
/obj/structure/table/proc/dismantle(obj/item/wrench/W, mob/user)
	...
	user.visible_message(SPAN("notice", "\The [user] begins dismantling \the [src]."),
	                              SPAN("notice", "You begin dismantling \the [src]."))
	playsound(src.loc, 'sound/items/Ratchet.ogg', 50, 1)
	if(!do_after(user, 20, src))
		manipulating = 0
		return
	user.visible_message(SPAN("notice", "\The [user] dismantles \the [src]."),
	                              SPAN("notice", "You dismantle \the [src]."))
