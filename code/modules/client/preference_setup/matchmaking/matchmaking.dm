...
/datum/matchmaker/proc/do_matchmaking()
	...
	for(var/mob/M in to_warn)
		to_chat(M ,SPAN("warning", "You have new connections. Use \"See Relationship Info\" to view and finalize them."))
...
/datum/relation/proc/sever()
	to_chat(holder.current ,SPAN("warning", "Your connection with [other.holder] is no more."))
	to_chat(other.holder.current ,SPAN("warning", "Your connection with [holder] is no more."))
	other.other = null
...
/datum/relation/proc/finalize()
	finalized = 1
	to_chat(holder.current ,SPAN("warning", "You have finalized a connection with [other.holder]."))
	to_chat(other.holder.current ,SPAN("warning", "[holder] has finalized a connection with you."))
	if(other && other.finalized)
		to_chat(holder.current ,SPAN("warning", "Your connection with [other.holder] is now confirmed!"))
		to_chat(other.holder.current ,SPAN("warning", "Your connection with [holder] is now confirmed!"))
