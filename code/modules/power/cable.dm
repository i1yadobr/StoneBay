...
/obj/structure/cable/attackby(obj/item/W, mob/user)
	...
	if(isWirecutter(W))
		if(d1 == UP || d2 == UP)
			to_chat(user, SPAN("warning", "You must cut this cable from above."))
			return

		if(breaker_box)
			to_chat(user, SPAN("warning", "This cable is connected to nearby breaker box. Use breaker box to interact with it."))
			return
		...
		for(var/mob/O in viewers(src, null))
			O.show_message(SPAN("warning", ")cuts the cable."), 1)
		...
		if(powernet && (powernet.avail > 0))		// is it powered?
			to_chat(user, SPAN("warning", "[get_wattage()] in power network."))

		else
			to_chat(user, SPAN("warning", "The cable is not powered."))
...
/obj/item/stack/cable_coil/proc/set_cable_color(selected_color, user)
	...
	color = final_color
	to_chat(user, SPAN("notice", "You change \the [src]'s color to [lowertext(selected_color)]."))
...
/obj/item/stack/cable_coil/verb/make_restraint()
	...
	if(ishuman(M) && !M.incapacitated())
		...
		if(src.amount <= 14)
			to_chat(usr, SPAN("warning", "You need at least 15 lengths to make restraints!"))
			return
		var/obj/item/handcuffs/cable/B = new /obj/item/handcuffs/cable(usr.loc)
		B.color = color
		to_chat(usr, SPAN("notice", "You wind some cable together to make some restraints."))
		src.use(15)
	else
		to_chat(usr, SPAN("notice", "You cannot do that."))
...
/obj/item/stack/cable_coil/proc/turf_place(turf/simulated/F, mob/user)
	...
	for(var/obj/structure/cable/LC in F)
		if((LC.d1 == dirn && LC.d2 == end_dir ) || ( LC.d2 == dirn && LC.d1 == end_dir))
			to_chat(user, SPAN("warning", "There's already a cable at that position."))
			return
