...
/datum/emote/proc/do_emote(mob/user, emote_key, intentional, target, additional_params)
	...
	if(msg_3p)
		if(message_type & VISIBLE_MESSAGE)
			user.visible_message(message = msg_3p, self_message = msg_1p, blind_message = impaired_msg, range = range, check_ghosts = /datum/client_preference/ghost_sight)
		else if(message_type & AUDIBLE_MESSAGE)
			user.audible_message(message = msg_3p, self_message = msg_1p, deaf_message = impaired_msg, hearing_distance = range, check_ghosts = /datum/client_preference/ghost_sight)
