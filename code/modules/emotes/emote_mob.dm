...
/mob/proc/get_emote(key)
	return AL_LAZYACCESS(current_emotes, key)

/mob/proc/set_emote(key, datum/emote/emote)
	AL_LAZYSET(current_emotes, key, emote)

/mob/proc/clear_emote(key)
	AL_LAZYREMOVE(current_emotes, key)
...
/mob/proc/custom_emote(message_type = VISIBLE_MESSAGE, message, intentional = FALSE)
	...
	if(message_type & VISIBLE_MESSAGE)
		visible_message(message, check_ghosts = /datum/client_preference/ghost_sight)
	else
		audible_message(message, check_ghosts = /datum/client_preference/ghost_sight)
