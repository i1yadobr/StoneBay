...
/obj/item/device/eftpos/attack_self(mob/user as mob)
	if(get_dist(src,user) <= 1)
		...
		if(transaction_locked)
			...
			dat += "Value: <a href='byond://?src=\ref[src];choice=trans_value'>[transaction_amount]cr.</a><br>"
...
/obj/item/device/eftpos/proc/scan_card(obj/item/card/I, obj/item/ID_container)
	if (istype(I, /obj/item/card/id))
		...
		if(I==ID_container || ID_container == null)
			usr.visible_message(SPAN("info", "\The [usr] swipes a card through \the [src]."))
		else
			usr.visible_message(SPAN("info", "\The [usr] swipes \the [ID_container] through \the [src]."))
		if(transaction_locked && !transaction_paid)
			if(linked_account)
				if(!linked_account.suspended)
					var/attempt_pin = ""
