...
/obj/machinery/mining/drill/attack_hand(mob/user)
	...
	if (panel_open && cell && user.Adjacent(src))
		...
		if(use_cell_power())
			...
			to_chat(user, SPAN("notice", "The drill is unpowered."))
	else
		to_chat(user, SPAN("notice", "Turning on a piece of industrial machinery without sufficient bracing or wires exposed is a bad idea."))
...
/obj/machinery/mining/drill/verb/unload()
	...
	if(B)
		for(var/obj/item/ore/O in contents)
			O.forceMove(B)
		to_chat(usr, SPAN("notice", "You unload the drill's storage cache into the ore box."))
	else
		to_chat(usr, SPAN("notice", "You must move an ore box up to the drill before you can unload it."))
...
/obj/machinery/mining/brace/attackby(obj/item/W as obj, mob/user as mob)
	if(connected && connected.active)
		to_chat(user, SPAN("notice", "You can't work with the brace of a running drill!"))
		return
	...
	if(isWrench(W))
		...
		if(istype(get_turf(src), /turf/space))
			to_chat(user, SPAN("notice", "You can't anchor something to empty space. Idiot."))
			return

		playsound(src.loc, 'sound/items/Ratchet.ogg', 100, 1)
		to_chat(user, SPAN("notice", "You [anchored ? "un" : ""]anchor the brace."))
