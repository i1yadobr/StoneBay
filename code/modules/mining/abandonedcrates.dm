...
/obj/structure/closet/crate/secure/loot/proc/generate_loot()
	...
	switch(loot)
		...
		if(77 to 78)
			new /obj/item/pickaxe/drill/diamond(src)
		if(79 to 80)
...
/obj/structure/closet/crate/secure/loot/togglelock(mob/user)
	...
	to_chat(user, SPAN("notice", "The crate is locked with a Deca-code lock."))
	var/input = input(user, "Enter [codelen] digits.", "Deca-Code Lock", "") as text
	if(!Adjacent(user))
		return

	if(input == null || length(input) != codelen)
		to_chat(user, SPAN("notice", "You leave the crate alone."))
	else if(check_input(input) && locked)
		to_chat(user, SPAN("notice", "The crate unlocks!"))
		...
		attempts--
		if (attempts == 0)
			to_chat(user, SPAN("danger", "The crate's anti-tamper system activates!"))
...
/obj/structure/closet/crate/secure/loot/attackby(obj/item/W, mob/user)
	if(locked)
		if (isMultitool(W)) // Greetings Urist McProfessor, how about a nice game of cows and bulls?
			to_chat(user, SPAN("notice", "DECA-CODE LOCK ANALYSIS:"))
			if (attempts == 1)
				to_chat(user, SPAN("warning", "* Anti-Tamper system will activate on the next failed access attempt."))
			else
				to_chat(user, SPAN("notice", "* Anti-Tamper system will activate after [src.attempts] failed access attempts."))
			if(lastattempt.len)
				var/bulls = 0
				var/cows = 0
				...
				to_chat(user, SPAN("notice", "Last code attempt had [bulls] correct digits at correct positions and [cows] correct digits at incorrect positions."))
			return
	..()
