...
/datum/seed/proc/do_thorns(mob/living/carbon/human/target, obj/item/fruit, target_limb)
	...
	if(blocked >= 100 || (target.species && target.species.species_flags & (SPECIES_FLAG_NO_EMBED|SPECIES_FLAG_NO_MINOR_CUT)))
		to_chat(target, SPAN("danger", "\The [fruit]'s thorns scratch against the armour on your [affecting.name]!"))
		return
	...
	if(get_trait(TRAIT_CARNIVOROUS) >= 2)
		if(affecting)
			to_chat(target, SPAN("danger", "\The [fruit]'s thorns pierce your [affecting.name] greedily!"))
		else
			to_chat(target, SPAN("danger", "\The [fruit]'s thorns pierce your flesh greedily!"))
		damage = max(5, round(15*get_trait(TRAIT_POTENCY)/100, 1))
		has_edge = prob(get_trait(TRAIT_POTENCY)/2)
	else
		if(affecting)
			to_chat(target, SPAN("danger", "\The [fruit]'s thorns dig deeply into your [affecting.name]!"))
		else
			to_chat(target, SPAN("danger", "\The [fruit]'s thorns dig deeply into your flesh!"))
...
/datum/seed/proc/do_sting(mob/living/carbon/human/target, obj/item/fruit)
	...
	if(chems && chems.len && target.reagents)
		...
		if(affecting)
			to_chat(target, SPAN("danger", "You are stung by \the [fruit] in your [affecting.name]!"))
			for(var/rid in chems)
				var/injecting = min(5,max(1,get_trait(TRAIT_POTENCY)/5))
				target.reagents.add_reagent(rid,injecting)
		else
			to_chat(target, SPAN("danger", "Sharp spines scrape against your armour!"))
...
/datum/seed/proc/harvest(mob/user,yield_mod,harvest_sample,force_amount)
	...
	if(!force_amount && get_trait(TRAIT_YIELD) == 0 && !harvest_sample)
		if(istype(user)) to_chat(user, SPAN("danger", "You fail to harvest anything useful."))
	else
