...
/obj/item/sample/proc/merge_evidence(obj/item/sample/supplied, mob/user)
	...
	to_chat(user, SPAN("notice", "You transfer the contents of \the [supplied] into \the [src]."))
	return 1
...
/obj/item/sample/print/merge_evidence(obj/item/sample/supplied, mob/user)
	...
	SetName("[initial(name)] (combined)")
	to_chat(user, SPAN("notice", "You overlay \the [src] and \the [supplied], combining the print records."))
	return 1
...
/obj/item/sample/print/attack_self(mob/user)
	...
	if(H.gloves)
		to_chat(user, SPAN("warning", "Take \the [H.gloves] off first."))
		return

	to_chat(user, SPAN("notice", "You firmly press your fingertips onto the card."))
...
/obj/item/sample/print/attack(mob/living/M, mob/user)
	...
	if(H.gloves)
		to_chat(user, SPAN("warning", "\The [H] is wearing gloves."))
		return 1
...
/obj/item/forensics/sample_kit/proc/take_sample(mob/user, atom/supplied)
	...
	to_chat(user, SPAN("notice", "You transfer [S.evidence.len] [S.evidence.len > 1 ? "[evidence_type]s" : "[evidence_type]"] to \the [S]."))
...
/obj/item/forensics/sample_kit/afterattack(atom/A, mob/user, proximity)
	...
	if(can_take_sample(user, A))
		...
		to_chat(user, SPAN("warning", "You are unable to locate any [evidence_type]s on \the [A]."))
		. = ..()
