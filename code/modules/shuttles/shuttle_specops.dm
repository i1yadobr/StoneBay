	...
	spawn(2 SECONDS)
		if (!location)	//just arrived home
			for(var/turf/T in get_area_turfs(shuttle_area))
				...
				to_chat(M, SPAN("danger", "You have arrived at [GLOB.using_map.boss_name]. Operation has ended!"))
		else	//just left for the station
			launch_mauraders()
			for(var/turf/T in get_area_turfs(shuttle_area))
				var/mob/M = locate(/mob) in T
				to_chat(M, SPAN("danger", "You have arrived at [GLOB.using_map.station_name]. Commence operation!"))
