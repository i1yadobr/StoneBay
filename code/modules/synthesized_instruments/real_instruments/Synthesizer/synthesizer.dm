...
/obj/structure/synthesized_instrument/synthesizer/attackby(obj/item/O, mob/user, params)
	if (isWrench(O))
		if (!anchored && !isinspace())
			...
			to_chat(usr, SPAN("notice", " You begin to tighten \the [src] to the floor..."))
			if (do_after(user, 20, src))
				if(!anchored && !isinspace())
					user.visible_message( \
						"[user] tightens \the [src]'s casters.", \
						SPAN("notice", " You tighten \the [src]'s casters. Now it can be played again."), \
						SPAN("italics", "You hear ratchet."))
					src.anchored = 1
		else if(anchored)
			playsound(src.loc, 'sound/items/Ratchet.ogg', 50, 1)
			to_chat(usr, SPAN("notice", " You begin to loosen \the [src]'s casters..."))
			if (do_after(user, 40, src))
				if(anchored)
					user.visible_message( \
						"[user] loosens \the [src]'s casters.", \
						SPAN("notice", " You loosen \the [src]. Now it can be pulled somewhere else."), \
						SPAN("italics", "You hear ratchet."))
					src.anchored = 0
	else
		..()
