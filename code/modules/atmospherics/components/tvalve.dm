...
/obj/machinery/atmospherics/tvalve/attackby(obj/item/W as obj, mob/user as mob)
	...
	if (istype(src, /obj/machinery/atmospherics/tvalve/digital))
		to_chat(user, SPAN("warning", "You cannot unwrench \the [src], it's too complicated."))
		return 1
	var/datum/gas_mixture/int_air = return_air()
	var/datum/gas_mixture/env_air = loc.return_air()
	if ((int_air.return_pressure()-env_air.return_pressure()) > 2*ONE_ATMOSPHERE)
		to_chat(user, SPAN("warnng", "You cannot unwrench \the [src], it too exerted due to internal pressure."))
