...
/obj/machinery/atmospherics/portables_connector/attackby(obj/item/W as obj, mob/user as mob)
	if(!isWrench(W))
		return ..()
	if (connected_device)
		to_chat(user, SPAN("warning", "You cannot unwrench \the [src], dettach \the [connected_device] first."))
		return 1
	...
	if ((int_air.return_pressure()-env_air.return_pressure()) > 2*ONE_ATMOSPHERE)
		to_chat(user, SPAN("warning", "You cannot unwrench \the [src], it too exerted due to internal pressure."))
