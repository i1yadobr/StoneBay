...
/obj/item/scrying/attack_self(mob/user as mob)
	if((user.mind && !GLOB.wizards.is_antagonist(user.mind)))
		to_chat(user, SPAN("warning", "You stare into the orb and see nothing but your own reflection."))
		return

	to_chat(user, SPAN("info", "You can see... everything!")) // This never actually happens.
