...
/datum/language/proc/format_message(message, verb)
	return "[verb], [SPAN("message", "[SPAN("[colour]", "\"[capitalize(message)]\"")]")]"

/datum/language/proc/format_message_plain(message, verb)
	return "[verb], \"[capitalize(message)]\""

/datum/language/proc/format_message_radio(message, verb)
	return "[verb], [SPAN("[colour]", "\"[capitalize(message)]\"")]"
...
/mob/proc/hear_broadcast(datum/language/language, mob/speaker, speaker_name, message)
	if((language in languages) && language.check_special_condition(src))
		var/msg = "<i>[SPAN("game say", "[language.name], [SPAN("name", "[speaker_name]")] [message]")]</i>"
		to_chat(src, msg)
...
/mob/observer/ghost/hear_broadcast(datum/language/language, mob/speaker, speaker_name, message)
	if(speaker.name == speaker_name || antagHUD)
		to_chat(src, "<i>[SPAN("game say", "[language.name], [SPAN("name", "[speaker_name]")] ([ghost_follow_link(speaker, src)]) [message]")]</i>")
	else
		to_chat(src, "<i>[SPAN("game say", "[language.name], [SPAN("name", "[speaker_name]")] [message]")]</i>")
