...
/mob/living/bot/floorbot/GetInteractPanel()
	...
	. += "<br>Make singles pieces of metal into tiles when empty: <a href='byond://?src=\ref[src];command=make'>[maketiles ? "Yes" : "No"]</a>"
...
/mob/living/bot/floorbot/emag_act(remaining_charges, mob/user)
	...
	if(!emagged)
		emagged = 1
		if(user)
			to_chat(user, SPAN("notice", "The [src] buzzes and beeps."))
		return 1
...
/obj/item/storage/toolbox/mechanical/attackby(obj/item/stack/tile/floor/T, mob/user)
	if(!istype(T, /obj/item/stack/tile/floor))
		...
		return
	if(contents.len >= 1)
		to_chat(user, SPAN("notice", "They wont fit in as there is already stuff inside."))
		return
	if(user.s_active)
	...
	if(T.use(10))
		...
		user.pick_or_drop(B)
		to_chat(user, SPAN("notice", "You add the tiles into the empty toolbox. They protrude from the top."))
		qdel(src)
	else
		to_chat(user, SPAN("warning", "You need 10 floor tiles for a floorbot."))
	return
...
/obj/item/toolbox_tiles/attackby(obj/item/W, mob/user as mob)
	...
	if(isprox(W))
		...
		user.pick_or_drop(B)
		to_chat(user, SPAN("notice", "You add the sensor to the toolbox and tiles!"))
		qdel(src)
...
/obj/item/toolbox_tiles_sensor/attackby(obj/item/W, mob/user as mob)
	...
	if(istype(W, /obj/item/robot_parts/l_arm) || istype(W, /obj/item/robot_parts/r_arm))
		...
		to_chat(user, SPAN("notice", "You add the robot arm to the odd looking toolbox assembly! Boop beep!"))
		qdel(src)
	else if(istype(W, /obj/item/pen))
		var/t = sanitizeSafe(input(user, "Enter new robot name", name, created_name), MAX_NAME_LEN)
		if(!t)
