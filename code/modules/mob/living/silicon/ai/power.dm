...
/mob/living/silicon/ai/proc/handle_power_failure()
	...
	to_chat(src, SPAN("danger", "Main power lost. System switched to internal capacitor. Beginning diagnostics."))
	...
	while(aiRestorePowerRoutine)
		...
		if(self_shutdown)
			to_chat(src, SPAN("notice", "Systems offline. Power restoration routine aborted."))
			aiRestorePowerRoutine = AI_RESTOREPOWER_IDLE
			return

		if(has_power(0))
			to_chat(src, SPAN("notice", "Main power restored. All systems returning to normal mode."))
			aiRestorePowerRoutine = AI_RESTOREPOWER_IDLE
			update_icon()
			return
		...
		switch(aiRestorePowerRoutine)
			if(AI_RESTOREPOWER_DIAGNOSTICS)
				to_chat(src, SPAN("notice", "Diagnostics completed. Failure confirmed: Main power connection nonfunctional."))
				continue
			if(AI_RESTOREPOWER_CONNECTING)
				to_chat(src, SPAN("notice", "Attempting to connect to area power controller."))
				continue
			...
			if(AI_RESTOREPOWER_CONNECTED)
				...
				if(!istype(theAPC))
					to_chat(src, SPAN("notice", "Error processing connection to APC: Attempt [connection_failures+1]/[AI_POWER_RESTORE_MAX_ATTEMPTS]"))
					connection_failures++
					if(connection_failures == AI_POWER_RESTORE_MAX_ATTEMPTS)
						aiRestorePowerRoutine = AI_RESTOREPOWER_FAILED
						to_chat(src, SPAN("danger", "Unable to connect to APC after [AI_POWER_RESTORE_MAX_ATTEMPTS] attempts. Aborting power restoration sequence."))
						continue
					aiRestorePowerRoutine = AI_RESTOREPOWER_CONNECTING
					continue
				to_chat(src, SPAN("notice", "APC connection confirmed: [theAPC]. Sending emergency reset signal..."))
				continue
			...
			if(AI_RESTOREPOWER_COMPLETED)
				...
				if(!istype(theAPC))
					to_chat(src, SPAN("danger", "Connection to APC lost. Attempting to re-connect."))
					aiRestorePowerRoutine = AI_RESTOREPOWER_CONNECTING
					connection_failures = 0
					continue
				// Our area has changed.
				if(get_area(src) != get_area(theAPC))
					to_chat(src, SPAN("danger", "APC change detected. Attempting to locate new APC."))
					aiRestorePowerRoutine = AI_RESTOREPOWER_CONNECTING
					connection_failures = 0
					continue
				// The APC is damaged
				if(theAPC.stat & BROKEN)
					to_chat(src, SPAN("danger", "APC internal diagnostics reports hardware failure. Unable to reset. Aborting power restoration sequence."))
					aiRestorePowerRoutine = AI_RESTOREPOWER_FAILED
					continue
				...
				if(theAPC.cell && CELL_PERCENT(theAPC.cell) < 1)
					to_chat(src, SPAN("danger", "APC internal power reserves are critical. Unable to restore main power."))
					aiRestorePowerRoutine = AI_RESTOREPOWER_FAILED
					continue
				// Success!
				to_chat(src, SPAN("notice", "Reset signal successfully transmitted. Sequence completed."))
				reset_apc(theAPC)
...
/mob/living/silicon/ai/proc/ai_shutdown()
	...
	if(self_shutdown)
		to_chat(src, SPAN("notice", "System rebooted. Camera, communication and network systems operational."))
		self_shutdown = 0
		return
	...
	if(confirm == "Yes")
		to_chat(src, SPAN("notice", "Shutting down. Minimal power mode: Enabled. You may reboot yourself by using the \"Shutdown\" command again."))
		self_shutdown = 1
		return
