...
/mob/living/silicon/robot/set_zeroth_law(law, law_borg)
	...
	if(tracking_entities)
		to_chat(src, SPAN("warning", "Internal camera is currently being accessed."))
...
/mob/living/silicon/proc/dostatelaws(method, prefix, datum/ai_laws/laws)
	if(stating_laws[prefix])
		to_chat(src, SPAN("notice", "[method]: Already stating laws using this communication method."))
		return
	...
	if(!can_state)
		to_chat(src, SPAN("danger", "[method]: Unable to state laws. Communication method unavailable."))
