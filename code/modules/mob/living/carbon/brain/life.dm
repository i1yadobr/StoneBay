...
/mob/living/carbon/brain/handle_regular_status_updates()	//TODO: comment out the unused bits >_>
	...
	if(is_ooc_dead())	//DEAD. BROWN BREAD. SWIMMING WITH THE SPESS CARP
		...
		if(!container && ((health < config.health.health_threshold_dead) || (world.time - timeofhostdeath > config.health.revival_brain_life ) && (config.health.revival_brain_life >= 0)))
			death()
			blinded = 1
			silent = 0
			return 1
