...
/mob/living/carbon/human/verb/quick_equip()
	...
	if(ishuman(src))
		...
		if(H.equip_to_appropriate_slot(I))
			...
			to_chat(H, SPAN("warning", "You are unable to equip that."))
...
/mob/living/carbon/human/equip_to_slot(obj/item/W, slot, redraw_mob = TRUE)
	...
	switch(slot)
		...
		if(slot_tie)
			...
			to_chat(src, SPAN("danger", "You are trying to eqip this item to an unsupported inventory slot. If possible, please write a ticket with steps to reproduce. Slot was: [slot]"))
			return
...
/mob/living/carbon/human/slot_is_accessible(slot, obj/item/I, mob/user=null)
	...
	if(covering && (covering.body_parts_covered & (I.body_parts_covered|check_flags)))
		to_chat(user, SPAN("warning", "\The [covering] is in the way."))
		return 0
	return 1
