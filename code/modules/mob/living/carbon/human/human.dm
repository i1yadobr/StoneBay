...
/mob/living/carbon/human/proc/show_inv_get_slots_reduced()
	...
	for(var/entry in species.hud.gear)
		...
		if(firstline)
			...
			data += "<BR>"
		data += "<B>[slot_ref["name"]]:</b> <a href='byond://?src=\ref[src];item=[slot_ref["slot"]]'>[istype(thing_in_slot) ? thing_in_slot : "nothing"]</a>"
	data += "<HR>"

	if(species.hud.has_hands)
		data += "<b>Left hand:</b> <A href='byond://?src=\ref[src];item=[slot_l_hand]'>[istype(l_hand) ? l_hand : "nothing"]</A>"
		data += "<BR><b>Right hand:</b> <A href='byond://?src=\ref[src];item=[slot_r_hand]'>[istype(r_hand) ? r_hand : "nothing"]</A>"
...
/mob/living/carbon/human/proc/show_inv_get_slots()
	...
	for(var/entry in species.hud.gear)
		...
		if(istype(thing_in_slot, /obj/item/clothing))
			...
			if(LAZYLEN(C.accessories))
				data += "<BR><A href='byond://?src=\ref[src];item=tie;holder=\ref[C]'>Remove accessory</A>"
	data += "<HR>"
	...
	if(istype(wear_mask, /obj/item/clothing/mask) || istype(head, /obj/item/clothing/head/helmet/space))
		if(istype(back, /obj/item/tank) || istype(belt, /obj/item/tank) || istype(s_store, /obj/item/tank))
			data += "<BR><A href='byond://?src=\ref[src];item=internals'>Toggle internals.</A>"

	var/obj/item/clothing/under/suit = w_uniform
	// Other incidentals.
	if(istype(suit))
		data += "<BR><b>Pockets:</b> <A href='byond://?src=\ref[src];item=pockets'>Empty or Place Item</A>"
		if(suit.rolled_down != -1)
			data += "<BR><A href='byond://?src=\ref[src];item=rolldown'>Roll Down Jumpsuit</A>"
		if(suit.has_sensor == 1)
			data += "<BR><A href='byond://?src=\ref[src];item=sensors'>Set sensors</A>"
	if(handcuffed)
		data += "<BR><A href='byond://?src=\ref[src];item=[slot_handcuffed]'>Handcuffed</A>"
...
/mob/living/carbon/human/proc/show_inv_get_underwear()
	...
	for(var/entry in worn_underwear)
		...
		data += "<BR><a href='byond://?src=\ref[src];item=\ref[UW]'>Remove \the [UW]</a>"
...
/mob/living/carbon/human/proc/show_inv_get_ending(mob/user, underwear_only = FALSE)
	...
	if(!underwear_only && user.IsAdvancedToolUser(TRUE))
		data += "<BR><A href='byond://?src=\ref[src];item=splints'>Remove splints</A>"
	data += "<HR><A href='byond://?src=\ref[src];refresh=1;underwear_only=[underwear_only]'>Refresh</A>"
	data += "<BR><A href='byond://?src=\ref[user];inv_close=1'>Close</A>"
...
/mob/living/carbon/human/proc/handle_embedded_and_stomach_objects()
	for(var/obj/item/organ/external/organ in src.external_organs)
		if(organ.splinted)
			continue
		for(var/obj/item/O in organ.implants)
