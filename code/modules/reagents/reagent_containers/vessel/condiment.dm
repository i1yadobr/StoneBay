	...
	if(istype(W, /obj/item/pen) || istype(W, /obj/item/device/flashlight/pen))
		...
		if(length(tmp_label) > 10)
			...
			if(length(tmp_label))
				...
				to_chat(user, SPAN("notice", "You remove the label."))
				label_text = null
				on_reagent_change()
		return
...
/obj/item/reagent_containers/vessel/condiment/afterattack(obj/target, mob/user, proximity)
	...
	if(istype(target, /obj/item/reagent_containers/food)) // These are not opencontainers but we can transfer to them
		if(!reagents || !reagents.total_volume)
			to_chat(user, SPAN("notice", "There is no condiment left in \the [src]."))
			return

		if(!target.reagents.get_free_space())
			to_chat(user, SPAN("notice", "You can't add more condiment to \the [target]."))
			return

		var/trans = reagents.trans_to_obj(target, amount_per_transfer_from_this)
		to_chat(user, SPAN("notice", "You add [trans] ml of the condiment to \the [target]."))
	else
		..()

/obj/item/reagent_containers/vessel/condiment/self_feed_message(mob/user)
	to_chat(user, SPAN("notice", "You swallow some of contents of \the [src]."))
