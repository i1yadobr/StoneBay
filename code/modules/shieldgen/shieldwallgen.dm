...
/obj/machinery/shieldwallgen/attack_hand(mob/user as mob)
	if(anchored != 1)
		to_chat(user, SPAN("warning", "The shield generator needs to be firmly secured to the floor first."))
		return 1
	if(src.locked && !istype(user, /mob/living/silicon))
		to_chat(user, SPAN("warning", "The controls are locked!"))
		return 1
	if(power != 1)
		to_chat(user, SPAN("warning", "The shield generator needs to be powered by wire underneath."))
		return 1
