	...
	switch(type_to_use)
		if("string")
			...
			if(istext(new_data) && user.IsAdvancedToolUser())
				O.data = new_data
				to_chat(user, SPAN("notice", "You set \the [src]'s memory to [O.display_data(O.data)]."))
		if("number")
			accepting_refs = FALSE
		...
		if("ref")
			accepting_refs = TRUE
			to_chat(user, SPAN("notice", "You turn \the [src]'s ref scanner on.  Slide it across \
...
/obj/item/integrated_circuit/memory/constant/afterattack(atom/target, mob/living/user, proximity)
	...
	if(accepting_refs && proximity)
		...
		to_chat(user, SPAN("notice", "You set \the [src]'s memory to a reference to [O.display_data(O.data)].  The ref scanner is \
		now off."))
		accepting_refs = FALSE
