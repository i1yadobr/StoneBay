...
/obj/item/modular_computer/proc/run_program(prog)
	...
	if(!P || !istype(P)) // Program not found or it's not executable program.
		to_chat(user, SPAN("danger", "\The [src]'s screen shows \"I/O ERROR - Unable to run [prog]\" warning."))
		return
	...
	if(idle_threads.len >= processor_unit.max_idle_programs+1)
		to_chat(user, SPAN("notice", "\The [src] displays a \"Maximal CPU load reached. Unable to run another program.\" error"))
		return

	if(P.requires_ntnet && !get_ntnet_status(P.requires_ntnet_feature)) // The program requires NTNet connection, but we are not connected to NTNet.
		to_chat(user, SPAN("danger", "\The [src]'s screen shows \"NETWORK ERROR - Unable to connect to NTNet. Please retry. If problem persists contact your system administrator.\" warning."))
		return
