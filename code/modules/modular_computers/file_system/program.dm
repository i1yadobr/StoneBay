...
/datum/computer_file/program/proc/is_supported_by_hardware(hardware_flag = 0, loud = 0, mob/user = null)
	if(!(hardware_flag & usage_flags))
		if(loud && computer && user)
			to_chat(user, SPAN("warning", "\The [computer] flashes: \"Hardware Error - Incompatible software\"."))
		return 0
	return 1
...
/datum/computer_file/program/proc/can_run(mob/living/user, loud = 0, access_to_check)
	...
	if(!I)
		if(loud)
			to_chat(user, SPAN("notice", "\The [computer] flashes an \"RFID Error - Unable to scan ID\" warning."))
		return 0

	if(access_to_check in I.access)
		return 1
	else if(loud)
		to_chat(user, SPAN("notice", "\The [computer] flashes an \"Access Denied\" warning."))
