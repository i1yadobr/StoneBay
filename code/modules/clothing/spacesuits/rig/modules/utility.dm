...
/obj/item/rig_module/device/drill
	...
	device_type = /obj/item/pickaxe/drill/diamond
...
/obj/item/rig_module/chem_dispenser/accepts_item(obj/item/input_item, mob/living/user)
	...
	if(total_transferred)
		to_chat(user, SPAN("info", "You transfer [total_transferred] ml into the suit reservoir."))
	else
		to_chat(user, SPAN("danger", "None of the reagents seem suitable."))
	return 1
...
/obj/item/rig_module/chem_dispenser/engage(atom/target)
	...
	if(!charge_selected)
		to_chat(H, SPAN("danger", "You have not selected a chemical type."))
		return 0
	...
	var/chems_to_use = 10
	if(charge.charges <= 0)
		to_chat(H, SPAN("danger", "Insufficient chems!"))
		return 0
	...
	if(target_mob != H)
		to_chat(H, SPAN("danger", "You inject [target_mob] with [chems_to_use] ml of [charge.display_name]."))
	to_chat(target_mob, SPAN("danger", "You feel a rushing in your veins as [chems_to_use] ml of [charge.display_name] [chems_to_use == 1 ? "is" : "are"] injected."))
...
/obj/item/rig_module/voice/engage()
	...
	switch(choice)
		if("Enable")
			...
			to_chat(usr, SPAN("info", "You enable the speech synthesiser."))
		if("Disable")
			active = 0
			voice_holder.active = 0
			to_chat(usr, SPAN("info", "You disable the speech synthesiser."))
		if("Set Name")
			var/raw_choice = sanitize(input(usr, "Please enter a new name.")  as text|null, MAX_NAME_LEN)
			if(!raw_choice)
				return 0
			voice_holder.voice = raw_choice
			to_chat(usr, SPAN("info", "You are now mimicking <B>[voice_holder.voice]</B>."))
	return 1
...
/obj/item/rig_module/device/stamp/engage(atom/target)
	...
	if(!target)
		if(device == iastamp)
			device = deniedstamp
			to_chat(holder.wearer, SPAN("notice", "Switched to denied stamp."))
		else if(device == deniedstamp)
			device = iastamp
			to_chat(holder.wearer, SPAN("notice", "Switched to internal affairs stamp."))
		return 1
