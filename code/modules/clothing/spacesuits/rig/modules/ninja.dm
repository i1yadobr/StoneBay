...
/obj/item/rig_module/teleporter/engage(atom/target, notify_ai)
	...
	if(!istype(H.loc, /turf))
		to_chat(H, SPAN("warning", "You cannot teleport out of your current location."))
		return 0
	...
	if(!T)
		to_chat(H, SPAN("warning", "No valid teleport target found."))
		return 0

	if(T.density)
		to_chat(H, SPAN("warning", "You cannot teleport into solid walls."))
		return 0

	if(T.z in GLOB.using_map.get_levels_with_trait(ZTRAIT_SEALED))
		to_chat(H, SPAN("warning", "You cannot use your teleporter on this Z-level."))
		return 0

	if(T.contains_dense_objects())
		to_chat(H, SPAN("warning", "You cannot teleport to a location with solid objects."))
		return 0

	if(T.z != H.z || get_dist(T, get_turf(H)) > world.view)
		to_chat(H, SPAN("warning", "You cannot teleport to such a distant object."))
		return 0
...
/obj/item/rig_module/self_destruct/engage(skip_check = FALSE)
	...
	if(!skip_check)
		...
		if(usr == holder.wearer)
			holder.wearer.visible_message(SPAN("warning", " \The [src.holder.wearer] flicks a small switch on the back of \the [src.holder]."))
			sleep(blink_delay)
