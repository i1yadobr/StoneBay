...
/obj/item/rig_module/proc/engage()
	...
	if(damage >= 2)
		to_chat(usr, SPAN("warning", "The [interface_name] is damaged beyond use!"))
		return 0

	if(world.time < next_use)
		to_chat(usr, SPAN("warning", "You cannot use the [interface_name] again so soon."))
		return 0

	if(!holder || holder.canremove)
		to_chat(usr, SPAN("warning", "The suit is not initialized."))
		return 0

	if(usr.lying || usr.stat || usr.stunned || usr.paralysis || usr.weakened)
		to_chat(usr, SPAN("warning", "You cannot use the suit in this state."))
		return 0

	if(holder.wearer && holder.wearer.lying)
		to_chat(usr, SPAN("warning", "The suit cannot function while the wearer is prone."))
		return 0

	if(holder.security_check_enabled && !holder.check_suit_access(usr))
		to_chat(usr, SPAN("danger", "Access denied."))
		return 0
