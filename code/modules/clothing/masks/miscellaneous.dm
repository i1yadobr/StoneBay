...
/obj/item/clothing/mask/ai/proc/engage_mask(mob/user)
	...
	eye.possess(user)
	to_chat(eye.owner, SPAN("notice", "You feel disorented for a moment as your mind connects to the camera network."))

/obj/item/clothing/mask/ai/proc/disengage_mask(mob/user)
	if(user == eye.owner)
		to_chat(eye.owner, SPAN("notice", "You feel disorented for a moment as your mind disconnects from the camera network."))
