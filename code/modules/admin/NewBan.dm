...
/proc/AddBan(ckey, computerid, reason, bannedby, temp, minutes, address)
	...
	if ( Banlist.dir.Find("[ckey][computerid]") )
		to_chat(usr, SPAN("warning", "Ban already exists."))
		return 0
	else if (!ckey)
		to_chat(usr, SPAN("warning", "Ckey does not exist."))
		return 0
	else
...
/datum/admins/proc/unbanpanel()
	...
	for (var/A in Banlist.dir)
		...
		dat += text("<meta charset=\"utf-8\"><tr><td><A href='byond://?src=[ref];unbanf=[key][id]'>(U)</A><A href='byond://?src=[ref];unbane=[key][id]'>(E)</A> Key: <B>[key]</B></td><td>ComputerID: <B>[id]</B></td><td>IP: <B>[ip]</B></td><td> [expiry]</td><td>(By: [by])</td><td>(Reason: [reason])</td></tr>")
