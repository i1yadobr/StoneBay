...
/proc/_DB_ban_record(a_ckey, a_computerid, a_ip, bantype, mob/banned_mob, duration = -1, reason, job = "", rounds = 0, banckey = null, banip = null, bancid = null)
	...
	var/setter = a_ckey
	if(usr)
		to_chat(usr, SPAN("notice", "Ban saved to database."))
...
/datum/admins/proc/DB_ban_unban(ckey, bantype, job = "")
	...
	if(ban_number == 0)
		to_chat(usr, SPAN("warning", "Database update failed due to no bans fitting the search criteria. If this is not a legacy ban you should contact the database admin."))
		return

	if(ban_number > 1)
		to_chat(usr, SPAN("warning", "Database update failed due to multiple bans fitting the search criteria. Note down the ckey, job and current time and contact the database admin."))
		return

	if(istext(ban_id))
		ban_id = text2num(ban_id)
	if(!isnum(ban_id))
		to_chat(usr, SPAN("warning", "Database update failed due to a ban ID mismatch. Contact the database admin."))
		return
...
/datum/admins/proc/DB_ban_unban_by_id(id)
	...
	if(ban_number == 0)
		to_chat(usr, SPAN("warning", "Database update failed due to a ban id not being present in the database."))
		return

	if(ban_number > 1)
		to_chat(usr, SPAN("warning", "Database update failed due to multiple bans having the same ID. Contact the database admin."))
		return
...
/datum/admins/proc/DB_ban_panel(playerckey = null, adminckey = null, playerip = null, playercid = null, dbbantype = null, match = null)
	...
	output += "<form method='GET' action='byond://?src=\ref[src]'><input type='hidden' name='src' value='\ref[src]'>"
	...
	output += "<form method='GET' action='byond://?src=\ref[src]'>"
