...
/proc/check_rights(rights_required, show_msg=1, client/C = usr)
	...
	if(!C.holder)
		if(show_msg)
			to_chat(C, SPAN("warning", "Error: You are not an admin."))
		return FALSE
	...
	if(rights_required)
		if(rights_required & C.holder.rights)
			...
			if(show_msg)
				to_chat(C, SPAN("warning", "Error: You do not have sufficient rights to do that. You require one of the following flags:[rights2text(rights_required," ")]."))
			return FALSE
	else
		return TRUE
...
/client/proc/stealth()
	...
	if(!holder)
		to_chat(src, SPAN("warning", "Error: You are not an admin."))
		return

	holder.stealthy_ = holder.stealthy_ == STEALTH_OFF ? STEALTH_MANUAL : STEALTH_OFF
	if(holder.stealthy_)
		to_chat(src, SPAN("notice", "You are now stealthed."))
	else
		to_chat(src, SPAN("notice", "You are no longer stealthed."))
