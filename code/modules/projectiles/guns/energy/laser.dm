...
/obj/item/gun/energy/laser/mounted/cyborg/afterattack(atom/A, mob/living/user)
	if (locked)
		to_chat(user, SPAN("warning", "Current security protocols are not allowing you to use [src]."))
		return
	..()
...
/obj/item/gun/energy/laser/practice/emag_act(remaining_charges, mob/user, emag_source)
	if(hacked())
		return NO_EMAG_ACT
	to_chat(user, SPAN("warning", "You disable the safeties on [src] and crank the output to the lethal levels."))
...
/obj/item/gun/energy/laser/practice/handle_post_fire(mob/user, atom/target, pointblank=0, reflex=0)
	...
	if(hacked())
		...
		if(!max_shots) //uh hoh gig is up
			to_chat(user, SPAN("danger", "\The [src] sizzles in your hands, acrid smoke rising from the firing end!"))
...
/obj/item/gun/energy/lasertag/special_check(mob/living/carbon/human/M)
	if(ishuman(M))
		if(!istype(M.wear_suit, required_vest))
			to_chat(M, SPAN("warning", "You need to be wearing your laser tag vest!"))
			return 0
	return ..()
