...
/obj/item/gun
	...
	var/burst = 1
	var/fire_delay = 0.6 SECONDS 	//delay after shooting before the gun can be used again
	var/burst_delay = 0.2 SECONDS	//delay between shots, if firing in bursts
	var/move_delay = 1
	...
	if(wielded_item_state)
		...
		if(istype(M))
			if(M.can_wield_item(src) && is_held_twohanded(M))
				AL_LAZYSET(item_state_slots, slot_l_hand_str, wielded_item_state)
				AL_LAZYSET(item_state_slots, slot_r_hand_str, wielded_item_state)
				improper_held_icon = TRUE
			else
				AL_LAZYSET(item_state_slots, slot_l_hand_str, (base_icon_state || initial(item_state)))
				AL_LAZYSET(item_state_slots, slot_r_hand_str, (base_icon_state || initial(item_state)))
