	...
	if(isScrewdriver(W))
		...
		if(T.welding)
			if(health < maxhealth)
				...
				to_chat(user, SPAN("notice", "[src] does not need a repair."))
		else
			to_chat(user, SPAN("notice", "Unable to repair while [src] is off."))
...
/obj/vehicle/emag_act(remaining_charges, mob/user as mob)
	if(!emagged)
		...
		if(locked)
			locked = 0
			to_chat(user, SPAN("warning", "You bypass [src]'s controls."))
		return 1
...
/obj/vehicle/proc/insert_cell(obj/item/cell/C, mob/living/carbon/human/H)
	...
	cell = C
	powercheck()
	to_chat(usr, SPAN("notice", "You install [C] in [src]."))

/obj/vehicle/proc/remove_cell(mob/living/carbon/human/H)
	if(!cell)
		return

	to_chat(usr, SPAN("notice", "You remove [cell] from [src]."))
