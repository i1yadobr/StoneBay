...
/datum/controller/master/Recover()
	...
	if(istype(BadBoy))
		...
		if(msg)
			to_chat(GLOB.admins, SPAN("boldannounce", "[msg]"))
			log_to_dd(msg)
	...
	if (istype(Master.subsystems))
		...
		to_chat(world, SPAN("boldannounce", "The Master Controller is having some issues, we will need to re-initialize EVERYTHING"))
		Initialize(20, TRUE)
