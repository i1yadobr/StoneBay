	...
	if(href_list["add_antagonist"])
		...
		if(antag)
			if(antag.add_antagonist(src, 1, 1, 0, 1, 1)) // Ignore equipment and role type for this.
				...
				to_chat(usr, SPAN("warning", "[src] could not be made into a [antag.role_text]!"))
		...
		if(mind)
			...
			if(new_ambition)
				to_chat(mind.current, SPAN("warning", "Your ambitions have been changed by higher powers, they are now: [mind.ambitions]"))
				log_and_message_admins("made [key_name(mind.current)]'s ambitions be '[mind.ambitions]'.")
			else
				to_chat(mind.current, SPAN("warning", "Your ambitions have been unmade by higher powers."))
				log_and_message_admins("has cleared [key_name(mind.current)]'s ambitions.")
		else
			to_chat(usr, SPAN("warning", "The mind has ceased to be."))
				...
				for(var/obj/item/implant/loyalty/I in H.contents)
					for(var/obj/item/organ/external/organs in H.organs)
						if(I in organs.implants)
							qdel(I)
							break
				to_chat(H, SPAN("notice", "<font size =3><B>Your loyalty implant has been deactivated.</B></font>"))
				log_admin("[key_name_admin(usr)] has de-loyalty implanted [current].")
			if("add")
				to_chat(H, SPAN("danger", "<font size =3>You somehow have become the recepient of a loyalty transplant, and it just activated!</font>"))
		...
		var/obj_count = 1
		to_chat(current, SPAN("notice", "Your current objectives:"))
		for(var/datum/objective/objective in objectives)
